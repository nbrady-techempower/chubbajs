...
const { config, pool } = require("../server").context;
...
class Model {
  ...
  async getById(id) {
    let res = await pool.query(`SELECT * FROM "${this.tableName}" WHERE id = $1::int`, [id]);
    if (res.rows[0]) {
  ...
  async save() {
    // TODO: If there is no ID this is an insert statement

    // These are all the keys that need to be saved
    const colKeysToSave = Object.keys(this.model.columns).filter(k => k !== "id" && typeof this[k] !== "undefined");
    ...
    if (this.id) {
      ...
      updateStr += colKeysToSave.map((k, i) => `"${k}" = $${i + 1}::` + this.model.columns[k].driverType).join(",");
      ...
      const valStr = colKeysToSave.map((k, i) => `$${i+1}::` + this.model.columns[k].driverType).join(",");
